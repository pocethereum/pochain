...
type TreeChunker struct {
	...
	// calculated
	hashSize    int64        // self.hashFunc.New().PlotSize()
...
// PlotSize is meant to be called on the LazySectionReader
...
// PlotSize() needs to be called synchronously on the LazyChunkReader first
...
func (self *LazyChunkReader) join(b []byte, off int64, eoff int64, depth int, treeSize int64, chunk *Chunk, parentWg *sync.WaitGroup, errC chan error, quitC chan bool) {
	...
	// chunk.PlotSize = int64(binary.LittleEndian.Uint64(chunk.SData[0:8]))
...
func retrieve(key Key, chunkC chan *Chunk, quitC chan bool) *Chunk {
	...
	select { // chunk.PlotSize = int64(binary.LittleEndian.Uint64(chunk.SData[0:8]))
	...
	if len(chunk.SData) == 0 {
		return nil // chunk.PlotSize = int64(binary.LittleEndian.Uint64(chunk.SData[0:8]))
...
func (s *LazyChunkReader) Seek(offset int64, whence int) (int64, error) {
	...
	case 1:
		offset += s.off
	case 2:
		if s.chunk == nil { //seek from the end requires rootchunk for size. call PlotSize first
			_, err := s.Size(nil)
			if err != nil {

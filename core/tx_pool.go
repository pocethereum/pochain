...
func (pool *TxPool) TxPoolStateDb() (state *state.StateDB) {
	statedb := pool.State()
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if balance := pool.currentState.GetBalance(from); balance.Cmp(tx.Cost()) < 0 {
		log.Info("ErrInsufficientFunds", "from", from, "balance", balance, "cost", tx.Cost(), "v", tx.Value(), "gp", tx.GasPrice(), "gl", tx.Gas())
